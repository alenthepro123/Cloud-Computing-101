...
jujutsu_icon_url = 'https://scontent.fcgy1-1.fna.fbcdn.net/v/t1.15752-9/446078434_1144381083474619_7285188885118985341_n.png?_nc_cat=103&ccb=1-7&_nc_sid=5f2048&_nc_eui2=AeGnOTm4CwoixtmPE58Hg2xwyHdxc74c97vId3Fzvhz3u3tLPLvKbi0X7z0S8nEMxlh_8Mp7bds9rrh5A8Zq5dWb&_nc_ohc=evpyxeTAa7sQ7kNvgHtHV1Y&_nc_ht=scontent.fcgy1-1.fna&oh=03_Q7cD1QHjoGdt1Hm8GhYLwvuV-KE9u9IojcEmhmbKdHckdzrtlA&oe=668D64B1'
st.set_page_config(page_title="Anime Recommendation System", page_icon=jujutsu_icon_url)

# Load the dataset
@st.cache_data
def load_data():
    # Use relative path
    file_path = os.path.join(os.path.dirname(__file__), 'anime-dataset-2023.csv')
    try:
...
def get_top_anime(genre, studio):
    filtered_data = df[df['Genres'].str.contains(genre, na=False) & df['Studios'].str.contains(studio, na=False)]
...
# Select Studio
studios = sorted(set(studio for sublist in df['Studios'].dropna().str.split(', ') for studio in sublist))
